...
export function LinkHandler(room:Room):void {
    if (Game.time % 2 === 0 && room.controller != undefined && room.controller.my && room.memory.roomLevel === 2) { 
        ...
        const controllerLink:StructureLink|null = _.filter(unpackPosition(room.memory.layout.controllerStore).lookFor(LOOK_STRUCTURES),(s:Structure)=>(s.structureType === STRUCTURE_LINK))[0] as StructureLink;
        if (controllerLink === null) {
            return;
        }
        const cPos = unpackPosition(room.memory.layout.baseCenter);
        const lPos = new RoomPosition(cPos.x,cPos.y-1,cPos.roomName);
        const baseLink:StructureLink|null = _.filter(lPos.lookFor(LOOK_STRUCTURES),(s:Structure)=>(s.structureType === STRUCTURE_LINK))[0] as StructureLink;
        if (baseLink === null) {
            return;
        }

        const minerLinks:StructureLink[] = _.filter(room.find(FIND_MY_STRUCTURES),(s:Structure) => (
        ...
        if (room.controller!.ticksToDowngrade < 20000) {
            ...
            room.memory.linkStatus = "empty";
        } else if (storage.store.getUsedCapacity(RESOURCE_ENERGY) > C.BASE_LINK_GREEDY_LIMIT && controllerLink != undefined && controllerLink.store.getFreeCapacity(RESOURCE_ENERGY) as number >= 25)  {
        ...
        if (room.memory.linkStatus === "fill") {
            ...
            if (minerLinks.length > 0) {
                if (minerLinks[0].cooldown === 0 && minerLinks[0].store.getUsedCapacity(RESOURCE_ENERGY) as number >= eNeeded) {
                    minerLinks[0].transferEnergy(controllerLink);
                    transfered = true;
                } else {
                    if (minerLinks.length > 1) {
                        if (minerLinks[1].cooldown === 0 && minerLinks[1].store.getUsedCapacity(RESOURCE_ENERGY) as number >= eNeeded) {
