...
function updateSpawnQueue(room: Room):void {
    ...
    if (room.memory.spawnQueue.length > 0) {
        if (room.memory.spawnQueue[0].energy != undefined &&room.memory.spawnQueue[0].energy! > room.energyCapacityAvailable) {
    ...
    if (creeps.length === 0) {
        ...
        if (room.memory.remoteSupportRooms.length > 0) {
            for (const r of room.memory.remoteSupportRooms) {
                const fAmt = _.filter(Game.creeps, (c:Creep)=>(c.memory.role === "foot" && c.memory.home === r)).length + _.filter(room.memory.spawnQueue, (sd:SpawnData) => (sd.role === "foot" && sd.memory && sd.memory.home === r)).length;;
...
const roleCountFunctions:{[role:string] : RoleCountFunction} = {
    foot:function (room:Room, creeps:Creep[]):number {
        return _.filter(creeps, (c:Creep)=>(c.memory.role === "foot")).length + _.filter(room.memory.spawnQueue, (sd:SpawnData) => (sd.role === "foot")).length;
    },
    upgrader:function (room:Room, creeps:Creep[]):number {
        return _.filter(creeps, (c:Creep)=>(c.memory.role === "upgrader")).length + _.filter(room.memory.spawnQueue, (sd:SpawnData) => (sd.role === "upgrader")).length;
    },
    miner:function (room:Room, creeps:Creep[]):number {
        return _.filter(creeps, (c:Creep)=>(c.memory.role === "miner" && (c.ticksToLive === undefined || c.ticksToLive > 50))).length + _.filter(room.memory.spawnQueue, (sd:SpawnData) => (sd.role === "miner")).length;
    },
    builder:function (room:Room, creeps:Creep[]):number {
        return _.filter(creeps, (c:Creep)=>(c.memory.role === "builder")).length + _.filter(room.memory.spawnQueue, (sd:SpawnData) => (sd.role === "builder")).length;
    },
    filler:function (room:Room, creeps:Creep[]):number {
        return _.filter(creeps, (c:Creep)=>(c.memory.role === "filler")).length + _.filter(room.memory.spawnQueue, (sd:SpawnData) => (sd.role === "filler")).length;
    },
    hauler:function (room:Room, creeps:Creep[]):number {
        return _.filter(creeps, (c:Creep)=>(c.memory.role === "hauler")).length + _.filter(room.memory.spawnQueue, (sd:SpawnData) => (sd.role === "hauler")).length;
    },
    remoteMiner:function (room:Room, creeps:Creep[]):number {
        return _.filter(creeps, (c:Creep)=>(c.memory.role === "remoteMiner" && (c.ticksToLive === undefined || c.ticksToLive > 50))).length + _.filter(room.memory.spawnQueue, (sd:SpawnData) => (sd.role === "remoteMiner")).length;
    },
    remoteHauler:function (room:Room, creeps:Creep[]):number {
        return _.filter(creeps, (c:Creep)=>(c.memory.role === "remoteHauler")).length + _.filter(room.memory.spawnQueue, (sd:SpawnData) => (sd.role === "remoteHauler")).length;
    },
    reserver:function (room:Room, creeps:Creep[]):number {
        return _.filter(creeps, (c:Creep)=>(c.memory.role === "reserver")).length + _.filter(room.memory.spawnQueue, (sd:SpawnData) => (sd.role === "reserver")).length;
    }, 
    peacekeeper:function (room:Room, creeps:Creep[]):number {
        return _.filter(creeps, (c:Creep)=>(c.memory.role === "peacekeeper")).length + _.filter(room.memory.spawnQueue, (sd:SpawnData) => (sd.role === "peacekeeper")).length;
    },
    scout:function (room:Room, creeps:Creep[]):number {
        return _.filter(creeps, (c:Creep)=>(c.memory.role === "scout")).length + _.filter(room.memory.spawnQueue, (sd:SpawnData) => (sd.role === "scout")).length;
    },
    mineralMiner:function (room:Room, creeps:Creep[]):number {
        return _.filter(creeps, (c:Creep)=>(c.memory.role === "mineralMiner")).length + _.filter(room.memory.spawnQueue, (sd:SpawnData) => (sd.role === "mineralMiner")).length;
    },
    mineralHauler:function (room:Room, creeps:Creep[]):number {
        return _.filter(creeps, (c:Creep)=>(c.memory.role === "mineralHauler")).length + _.filter(room.memory.spawnQueue, (sd:SpawnData) => (sd.role === "mineralHauler")).length;
    },
    labrador:function (room:Room, creeps:Creep[]):number {
        return _.filter(creeps, (c:Creep)=>(c.memory.role === "labrador")).length + _.filter(room.memory.spawnQueue, (sd:SpawnData) => (sd.role === "labrador")).length;
    }
}
...
const roleNeedsFunctions:{[role:string] : RoleNeedFunction} = {
    ...
    upgrader:function(room:Room):number {
        ...
        if (room.storage === undefined) {
            ...
            return 1 + Math.floor( ((room.memory.resources != undefined)?room.memory.resources.total[RESOURCE_ENERGY]:0)/100000); 
        }
    },
    ...
    builder:function(room:Room):number {
        if (room.memory.constructionSites === undefined || room.memory.repairTargets === undefined || room.controller === undefined) {
            return 0;
        }
        const repNum = Math.max(Object.keys(room.memory.constructionSites).length,Object.keys(room.memory.repairTargets).length);
    ...
    hauler:function(room:Room):number {
        ...
        const container:StructureContainer = _.filter(cPos.lookFor(LOOK_STRUCTURES),(s:Structure)=>(s.structureType === STRUCTURE_CONTAINER))[0] as StructureContainer;
    ...
    remoteMiner:function(room:Room):number {
        ...
        for(let r of room.memory.remotes) {
            if (Memory.rooms[r] != undefined && Memory.rooms[r].remoteLayout != undefined && Memory.rooms[r].remoteLayout.sources != undefined) {
    ...
    mineralMiner:function (room:Room):number {
        ...
        if (room.memory.resources != undefined && room.memory.resources.total[RESOURCE_ENERGY] < C.MINERAL_MINING_ENERGY_NEEDED) {
            return 0;
        }
...
const roleCalcFunctions:{[role:string] : RoleCalcFunction} = {
    ...
    upgrader:function (room:Room, delta:number):void {
        if (delta < 0) {
            ...
            } else if (room.controller && room.controller.level === 7 && room.memory.resources && room.memory.resources.total[RESOURCE_ENERGY] >= C.FULL_UPGRADER_ENERGY_NEEDED) {
                room.memory.spawnQueue.push({
                    role:"upgrader",
                    pattern:"w40m5c5",
                    energy:room.energyCapacityAvailable
    ...
    miner:function (room:Room, delta:number):void {
        ...
        if (delta < 0) {
            for(let s in room.memory.layout.sources) {
                ...
                //console.log(s);
                const creeps:Creep[] = _.filter(Game.creeps, (c:Creep)=>(c.memory.home === room.name));
                const minerCount:number = _.filter(creeps, (c:Creep)=>(c.memory.role === "miner" && c.memory.roleData != undefined && c.memory.roleData.targetId === s && (c.ticksToLive === undefined || c.ticksToLive > 25+source.dist*3))).length + _.filter(room.memory.spawnQueue, (sd:SpawnData)=>(sd.role === "miner" && sd.memory != null && sd.memory.roleData != undefined && sd.memory.roleData.targetId === s)).length;
                if (minerCount === 0) {
                    const sqAmt = _.filter(room.memory.spawnQueue, (sd:SpawnData)=>(sd.role === "foot")).length;
                    if (sqAmt > 0) {
                        room.memory.spawnQueue.push({
    ...
    filler:function (room:Room, delta:number):void {
        ...
        if (fillerCount === 0 && creeps.length > 0) {
            ...
            if (room.memory.spawnQueue[0] !== undefined && room.memory.spawnQueue[0].role === "filler" && room.memory.spawnQueue[0].energy! <= Math.max(room.energyAvailable,300)) {
                return;
            }
            room.memory.spawnQueue.unshift({
                role:"filler",
    ...
    hauler:function (room:Room, delta:number):void {
        if (delta < 0) {
            for (let s in room.memory.layout.sources) {
                ...
                const link = _.filter(offsetPositionByDirection(offsetPositionByDirection(unpackPosition(sourceData.pos),sourceData.container),sourceData.link).lookFor(LOOK_STRUCTURES),(s:Structure)=>(s.structureType === STRUCTURE_LINK))[0];
                
                const creeps:Creep[] = _.filter(Game.creeps, (c:Creep)=>(c.memory.home === room.name));
                const haulerCount:number = _.filter(creeps, (c:Creep)=>(c.memory.role === "hauler" && c.memory.roleData != undefined && c.memory.roleData.targetId === s)).length + _.filter(room.memory.spawnQueue, (sd:SpawnData)=>(sd.role === "hauler" && sd.memory != null && sd.memory.roleData != undefined && sd.memory.roleData.targetId === s)).length;
        ...
        if (delta < 0) {
            ...
            for(let remote of room.memory.remotes) {
                for (let source in Memory.rooms[remote].remoteLayout.sources) {
                    ...
                    const remoteMinerCount:number = _.filter(creeps, (c:Creep)=>(c.memory.role === "remoteMiner" && c.memory.roleData != undefined && c.memory.roleData.target === remote && c.memory.roleData.targetId === source && (c.ticksToLive === undefined || c.ticksToLive > 34+s.dist*2))).length + _.filter(room.memory.spawnQueue, (sd:SpawnData)=>(sd.role === "remoteMiner" && sd.memory != null && sd.memory.roleData != undefined && sd.memory.roleData.target === remote && sd.memory.roleData.targetId === source)).length;
        ...
        if (delta < 0 || true) {
            ...
            for(let remote of room.memory.remotes) {
                for (let source in Memory.rooms[remote].remoteLayout.sources) {
                    ...
                    const remoteHaulerCount:number = _.filter(creeps, (c:Creep)=>(c.memory.role === "remoteHauler" && c.memory.roleData != undefined && c.memory.roleData.target === remote && c.memory.roleData.targetId === source)).length + _.filter(room.memory.spawnQueue, (sd:SpawnData)=>(sd.role === "remoteHauler" && sd.memory != null && sd.memory.roleData != undefined && sd.memory.roleData.target === remote && sd.memory.roleData.targetId === source)).length;
                    const tdist = sourceData.dist * 2;
    ...
    reserver:function (room:Room, delta:number):void {
        if (delta < 0) {
            ...
            for(let remote of room.memory.remotes) {
                ...
                const reserverCount:number = _.filter(creeps, (c:Creep)=>(c.memory.role === "reserver" && c.memory.roleData != undefined && c.memory.roleData.target === remote)).length + _.filter(room.memory.spawnQueue, (sd:SpawnData)=>(sd.role === "remoteHauler" && sd.memory != null && sd.memory.roleData != undefined && sd.memory.roleData.target === remote)).length;
                const reservation = Memory.rooms[remote].reservation;

                if (reserverCount === 0 && (reservation === undefined || reservation.username != "Awesomeadda" || reservation.ticksToEnd < 3000) ) {
                    room.memory.spawnQueue.push({
                        role:"reserver",
