...
function ResourceData(room:Room):void {
    ...
    let total:{[resourceType in ResourceConstant]:number} = {
        
    } as {[resourceType in ResourceConstant]:number};
    let delta:{[resourceType in ResourceConstant]:number} = {
    ...
    for (const resource of RESOURCES_ALL) {
        ...
        const f:StructureFactory = room.find(FIND_MY_STRUCTURES,{filter:(s)=>(s.structureType === STRUCTURE_FACTORY)})[0] as StructureFactory; 
        const factory = (f === undefined) ? 0 : f.store.getUsedCapacity(resource);
        
        const l:StructureLab[] = room.find(FIND_MY_STRUCTURES,{filter:(s)=>(s.structureType === STRUCTURE_LAB)}) as StructureLab[];
        const labs:number = (resource === RESOURCE_ENERGY) ? 0 : _.sum(l,function(lab:StructureLab) {
