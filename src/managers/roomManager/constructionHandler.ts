...
export function ConstructionHandler(room: OwnedRoom): void {
    ...
    for (const site of sites) {
        ...
        if (site.structureType === STRUCTURE_RAMPART) {
            energyNeed += Math.ceil((RAMPART_HITS_MAX[room.controller.level] * RAMPART_PERCENTAGE_MAX) / REPAIR_POWER);
        }
        siteMap[site.id] = 1;
    }
