    ...
    if (Game.time % 5 === 0 && room.controller != undefined && room.controller.my && room.memory.roomLevel === 2) {
        ...
        for(let r in room.memory.remotes) {
            ...
            if (Game.rooms[remote] != undefined) {
                remoteStructures = remoteStructures.concat(Game.rooms[remote].find(FIND_STRUCTURES,  {
        ...
        for (let structure of structures) {
            if (structure instanceof StructureWall || structure instanceof StructureRampart) {
                if (structure.hits < 2000 || (structure.hits < structure.hitsMax*C.RAMPART_PERCENTAGE_MIN && Object.keys(room.memory.constructionSites).length === 0)) {
                    room.memory.repairTargets[structure.id] = structure.pos;
                } else if (structure.hits > structure.hitsMax * C.RAMPART_PERCENTAGE_MAX || (Object.keys(room.memory.constructionSites).length > 0 && structure.hits > 2000)) {
