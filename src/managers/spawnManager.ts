                    ...
                    if (spawnData != undefined) {
                        ...
                        if (spawnData.body === undefined) {
                            if (spawnData.pattern != undefined && spawnData.energy != undefined) {
                                body = GenerateBodyFromPattern(spawnData.pattern,spawnData.energy).sort((a,b)=>(bodySortingValues[a]-bodySortingValues[b]));
                            } else {
                        ...
                        //const memory:CreepMemory = (spawnData.memory === undefined) ? {role:spawnData.role, home:room.name} : spawnData.memory;
                        
                        let name:string = (packPosition(new RoomPosition(25,25,room.name)) ).toString(16) + (Game.time%100000).toString(16);
                        name = name.replace('-','a');
