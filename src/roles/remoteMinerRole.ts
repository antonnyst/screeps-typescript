...
export class RemoteMinerRole extends CreepRole {
    runRole() {
        ...
        if (this.creep.pos.isEqualTo(minerPos)) {
            ...
            const container:StructureContainer = _.filter(minerPos.lookFor(LOOK_STRUCTURES),(s:Structure)=>(s.structureType === STRUCTURE_CONTAINER))[0] as StructureContainer;

            if (container == undefined || this.creep.store.getFreeCapacity(RESOURCE_ENERGY) > 0 || container.store.getFreeCapacity(RESOURCE_ENERGY) >= this.creep.getActiveBodyparts(WORK)*5) {
            ...
            if (this.creep.getActiveBodyparts(CARRY) > 0 && this.creep.store.getUsedCapacity(RESOURCE_ENERGY) > 0) {
                ...
                if (container === undefined) {
                    ...
                    let site = _.filter(minerPos.lookFor(LOOK_CONSTRUCTION_SITES),(s:Structure)=>(s.structureType === STRUCTURE_CONTAINER))[0];
                    if (site != undefined) {
