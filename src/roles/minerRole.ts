    ...
    runRole() {
        ...
        if (this.creep.pos.isEqualTo(minerPos)) {
            ...
            if (this.creep.getActiveBodyparts(CARRY) > 0 && this.creep.store.getUsedCapacity(RESOURCE_ENERGY) > 0) {
                const container:Structure|null = (this.creep.memory.roleData.anyStore.containerId === undefined) ? _.filter(minerPos.lookFor(LOOK_STRUCTURES),(s:Structure)=>(s.structureType === STRUCTURE_CONTAINER))[0] : Game.getObjectById(this.creep.memory.roleData.anyStore.containerId);
                ...
                if (container !== undefined && container !== null && this.creep.store.getUsedCapacity(RESOURCE_ENERGY) <= 20) {
                    this.creep.withdraw(container, RESOURCE_ENERGY);
                }

                if (this.creep.room.controller && this.creep.room.controller.level > 5) {
                    const link:StructureLink|null = (this.creep.memory.roleData.anyStore.linkId === undefined) ? _.filter(offsetPositionByDirection(minerPos,sourceData.link).lookFor(LOOK_STRUCTURES),(s:Structure)=>(s.structureType === STRUCTURE_LINK))[0] as StructureLink : Game.getObjectById(this.creep.memory.roleData.anyStore.linkId);
                    
                    if (link !== undefined && link !== null) {
                        this.creep.memory.roleData.anyStore.linkId = link.id;
                        if (link.store.getFreeCapacity(RESOURCE_ENERGY) as number > 0 && this.creep.store.getUsedCapacity(RESOURCE_ENERGY) >= 50-this.creep.getActiveBodyparts(WORK)*2) {
