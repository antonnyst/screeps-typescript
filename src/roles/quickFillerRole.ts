...
export class QuickFiller extends CreepRole {
    runRole() {
        ...
        const targets: (StructureExtension | StructureSpawn)[] = GetEnergyBuildings(
            ...
            this.creep.pos
        ).concat(GetEnergyBuildings(Memory.rooms[this.creep.memory.home].genBuildings!.extensions, this.creep.pos)) as (
            | StructureExtension
            | StructureSpawn
        )[];
        ...
        if (targets.length > 0 && this.creep.memory.targetPos === undefined) {
            ...
            const link = Game.getObjectById(Memory.rooms[this.creep.memory.home].genBuildings!.links[2].id!);
            if (link !== null && link instanceof StructureLink) {
                if (link.store.getFreeCapacity(RESOURCE_ENERGY) >= this.creep.store.getUsedCapacity(RESOURCE_ENERGY)) {
