...
export class MineralHaulerRole extends CreepRole {
    runRole() {
        ...
        if (this.creep.memory.roleData.hasEnergy == false && (this.creep.store.getFreeCapacity() == 0 || this.creep.room.find(FIND_MINERALS)[0].mineralAmount === 0)) {
        ...
        if (this.creep.memory.roleData.hasEnergy === false) {
            ...
            if (this.creep.pos.isNearTo(minerPos)) {
                ...
                if (container != undefined && container.store.getUsedCapacity(res) >= this.creep.store.getCapacity() && this.creep.ticksToLive! > 50) {
